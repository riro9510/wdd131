...
const temples = [
  ...
  temples.forEach(temple=>{
    temple.dedicated=convertDateFormat(temple.dedicated)
  });
   ...
   const displayTemples = (templesToShow) => {
    ...
    const dateFormat = formatDate(new Date(temple.dedicated));
    card.innerHTML = `
    <h3>${temple.templeName}</h3>
    <p><span class="label">Location:</span> ${temple.location}</p>
    <p><span class="label">Dedicated on:</span> ${dateFormat}</p>
    <p><span class="label">Area:</span> ${temple.area} sq ft</p>
    <img class="temples" src="${temple.imageUrl}" alt="${temple.templeName}" loading="lazy" width="400" height="250">
`;
